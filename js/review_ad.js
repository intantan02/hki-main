...
function loadData(page = 1, searchQuery = "") {
  ...
  fetch(url, { credentials: "same-origin" })
    ...
    .catch((err) => {
      ...
      if (table) table.innerHTML = `<tr><td colspan="9" class="text-danger">Error: ${escapeHtml(err.message || String(err))}</td></tr>`;
    });
}
...
function handleUpdateForm(form) {
  ...
  const statusCell = form.closest("tr") ? form.closest("tr").querySelector(".status-cell") : null;
  const sertifikatCell = form.closest("tr") ? form.closest("tr").querySelector(".sertifikat-cell") : null;
  ...
  fetch(updateUrl, {
    ...
    .then((json) => {
      if (json.success) {
        if (statusCell) statusCell.textContent = json.status || fd.get("status") || "—";
        if (json.fileName && sertifikatCell) {
          sertifikatCell.innerHTML = `<a href="../uploads/${encodeURIComponent(json.fileName)}" target="_blank" rel="noopener">Lihat</a>`;
        }
